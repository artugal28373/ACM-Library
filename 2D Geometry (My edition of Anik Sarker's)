...
namespace Vectorial {
    ...
    /// sorts by angle (counter-clock wise by radian)
        return modifiedatan2(a)<modifiedatan2(b);
    }
    double getDot (Vector a, Vector b) { return a.x * b.x + a.y * b.y; }
    double getCross (Vector a, Vector b) { return a.x * b.y - a.y * b.x; }
    double getLength (Vector a) { return sqrt(getDot(a, a)); }/// to get point a to point b distance, call getPLength(b-a)
...
namespace Linear {
    ...
    double closestPairPoint(Point* P, int n)
    {
		...
		{
			Point it= P[i];
			while(it.x-P[idx].x>ret){
				s.erase(P[idx]);idx++;
            }
			Point low= Point(it.x, it.y - ret);
			Point high= Point(it.x, it.y + ret);
    ...
    double perpendicularProjection(Point p, Point a, Point b){ /// from point p to line(a, b)
        Point edge= (b-a)/getLength(b-a);
        return getDot(edge, p-a);
    }
...
namespace Polygonal{
    ...
    double diameter(Point* p, int n, Point* ch)
    {
        ...
        {
            if(i==j)j= (j+1)%n;
            while(getDistance(ch[i], ch[j])<getDistance(ch[i], ch[ (j+1)%n ]))j= (j+1)%n;
            ret= max(ret, getDistance(ch[i], ch[j]));
        }
        return ret;
    }
    ...
    /// Tested on: CF 682E
    Polygon maximumEnclosingTriangle(Point* p, int n)/// p is a array containing Points of convexHull
    {
        Polygon ret;
        if(n<3)return ret;
    ...
    /// Tested on: UVA 12307
    pair<double, double>minimumEnclosingRectangle(Point *p, int n, Point *ch)
    {
        pair<double, double>ret= {1e9, 1e9};
        n= getConvexHull(p, n, ch);
        if(n<3)return ret;
        ...
        for(int i=0; i<n; i++)
        {
            while(perpendicularProjection(p[(r+1)%n], p[i], p[(i+1)%n])>perpendicularProjection(p[r%n], p[i], p[(i+1)%n]))r++;
            while(u<r || getDistanceToLine(p[(u+1)%n], p[i], p[(i+1)%n])>getDistanceToLine(p[u%n], p[i], p[(i+1)%n]))u++;
            while(l<u || perpendicularProjection(p[(l+1)%n], p[i], p[(i+1)%n])<perpendicularProjection(p[l%n], p[i], p[(i+1)%n]))l++;

            double w= perpendicularProjection(p[r%n], p[i], p[(i+1)%n])-perpendicularProjection(p[l%n], p[i], p[(i+1)%n]);
